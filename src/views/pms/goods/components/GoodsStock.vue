          ...
          >
            ...
            <el-table-column>
              ...
              <template slot-scope="{row}">
                <div v-for="item in row.values" style="margin-right:15px;display: inline-block">
                  <el-tag
                    closable
                  ...
                  <mini-card-upload v-if="row.index==0" v-model="item.picUrl" style="margin-top: 5px" />
                </div>
                <el-input
                ...
                <el-button
                  ...
                  @click="handleSpecValueAdd(row.index)"
                >添加规格值
                </el-button>
              </template>
            </el-table-column>
      ...
      this.$refs.specForm.validate((specValid) => {
        if (specValid) {
          this.$refs.skuForm.validate((skuValid) => {
            if (skuValid) {
              this.openFullScreen()
              const submitGoodsData = Object.assign({}, this.value)
              ...
              const skuList = JSON.parse(JSON.stringify(this.skuForm.skuList))
              skuList.map(item => {
                item.price *= 100
                return item
              })
