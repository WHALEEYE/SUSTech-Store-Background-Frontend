<template>
  <div class="app-container">
    <el-row :gutter="40">
      <el-col :span="24" :xs="30">
        <el-form ref="queryForm" :inline="true" :model="queryParams" label-width="68px" size="small">

          <el-form-item prop="search">
            <el-input
              v-model="searchNickname"
              clearable
              placeholder="Nickname"
              size="small"
              style="width: 240px"
              @keyup.enter.native="search"
            />
            <el-input
              v-model="searchPhoneNumber"
              clearable
              placeholder="PhoneNumber"
              size="small"
              style="width: 240px; margin-left: 10px"
              @keyup.enter.native="search"
            />
          </el-form-item>
          <!--          <el-form-item prop="status" style="margin-left: 100px">
                    </el-form-item>-->
          <el-form-item>
            <el-button icon="el-icon-search" type="primary" @click="search">search</el-button>
            <!--            <el-button icon="el-icon-refresh" @click="handleReset">重置</el-button>-->
          </el-form-item>
        </el-form>

        <el-table
          :v-loading="loading"
          element-loading-spinner="el-icon-loading"
          border
          highlight-current-row
          style="width: 100%"
          :data="pageList"
          @selection-change="handleSelectionChange"
        >
          <el-table-column v-if="false" align="center" label="id" prop="id" width="100" />
          <el-table-column align="center" label="Nickname" prop="nickname" width="150" fixed />
          <el-table-column align="center" label="Phone Number" prop="phoneNumber" width="150" />
          <el-table-column align="center" label="Card Number" prop="cardNumber" width="150" />
          <el-table-column align="center" label="Account Balance" prop="accountBalance" width="150" />
          <el-table-column
            v-if="false"
            align="center"
            label="Agent Service Notification"
            prop="agentServiceNotification"
            width="150"
          />
          <el-table-column
            v-if="false"
            align="center"
            label="Api Trade Notification"
            prop="apiTradeNotification"
            width="150"
          />
          <el-table-column v-if="false" align="center" label="Avatar Path" prop="avatarPath" width="150" />
          <el-table-column v-if="false" align="center" label="Alipay Account" prop="alipayAccount" width="150" />
          <el-table-column align="center" label="Banned" prop="banned" width="150" />
          <el-table-column align="center" label="Credit Score" prop="creditScore" width="150" />
          <el-table-column v-if="false" align="center" label="Follower Count" prop="followingCount" width="150" />
          <el-table-column align="center" label="Introduction" prop="introduction" width="200" />
          <el-table-column align="center" label="Notification" prop="secondHandNotification" width="150" />
          <el-table-column v-if="false" align="center" label="Sex" prop="sex" width="150" />
          <el-table-column align="center" label="Total Sold Value" prop="totalSoldValue" width="150" />
          <el-table-column align="center" label="Created Time" prop="createdTime" width="200" />
          <el-table-column align="center" label="Updated Time" prop="updatedTime" width="200" />
          <el-table-column align="center" label="Operation" width="100" fixed="right">
            <template slot-scope="scope">
              <el-button
                v-if="scope.row.banned==='false'"
                type="primary"
                icon="el-icon-close"
                size="mini"
                circle
                plain
                @click.stop="banUser(scope.row)"
              />
              <el-button
                v-if="scope.row.banned==='true'"
                type="primary"
                icon="el-icon-check"
                size="mini"
                circle
                plain
                @click.stop="unbanUser(scope.row)"
              />
            </template>
          </el-table-column>
        </el-table>

        <pagination
          v-show="pagination.total>0"
          :limit.sync="pagination.limit"
          :page.sync="pagination.page"
          :total="pagination.total"
          @pagination="loadData"
        />
      </el-col>
    </el-row>
    <!-- 新增或修改参数配置对话框 -->
    <el-dialog :title="dialog.title" :visible.sync="dialog.visible" width="800px">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="username" prop="username">
          <el-input v-model="form.username" placeholder="please input username" />
        </el-form-item>
        <el-form-item label="password" prop="password">
          <el-input v-model="form.password" placeholder="password: 6-20 numbers+letters" />
        </el-form-item>
        <el-form-item label="role">
          <el-select v-model="form.roleId" placeholder="please choose">
            <el-option label="Admin" :value="2" />
            <el-option label="Checker" :value="1" />
          </el-select>
        </el-form-item>

      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleSubmit">confirm</el-button>
        <el-button @click="dialog.visible=false">cancel</el-button>
      </div>
    </el-dialog>
  </div>
</template>

<script>
import { add, ban, list, unban } from '@/api/system/front'
import TreeSelect from '@riophae/vue-treeselect'
import '@riophae/vue-treeselect/dist/vue-treeselect.css'
import { removeToken } from '@/utils/auth'
import Pagination from '@/components/Pagination'

export default {
  components: { TreeSelect, Pagination },
  data() {
    return {
      searchNickname: '',
      searchPhoneNumber: '',
      // 遮罩层
      loading: true,
      // 选中数组
      id: undefined,
      // 非单个禁用
      single: true,
      // 非多个禁用
      multiple: true,
      queryParams: {
        username: undefined,
        mobile: undefined
      },
      pagination: {
        page: 1,
        limit: 10,
        total: 10
      },
      pageList: [],
      dialog: {
        title: undefined,
        visible: false
      },
      // 部门名称
      deptName: undefined,
      // 部门树选项
      deptOptions: undefined,
      // 角色选项
      roleOptions: [{ name: 'Super', id: 3 }, { name: 'Admin', id: 2 }, { name: 'Normal', id: 1 }],
      // 表单参数
      form: {
        roleIds: []
      },
      // 表单校验
      rules: {
        username: [
          { required: true, message: 'username can not be null', trigger: 'blur' }
        ],
        password: [
          { required: true, pattern: /^[a-zA-Z0-9]{6,20}$/, message: 'password is not valid', trigger: 'blur' }
        ],
        roleId: [
          { required: true, message: 'role can not be null', trigger: 'blur' }
        ]
      }
    }
  },
  watch: {
    // 根据名称筛选部门树
    deptName(val) {
      this.$refs.tree.filter(val)
    }
  },
  mounted() {
    this.loadData()
  },
  methods: {
    handleSelectionChange(selection) {
      this.id = selection.map(item => item.id)
      this.single = selection.length !== 1
      this.multiple = !selection.length
    },
    handleQuery() {
      this.queryParams.page = this.pagination.page
      this.queryParams.limit = this.pagination.limit
      list({}).then(response => {
        const { data, total } = response
        this.pageList = data
        this.pagination.total = total
        this.loading = false
      })
    },
    search() {
      list({
        pageNo: this.pagination.page,
        pageSize: this.pagination.limit,
        searchNickname: this.searchNickname,
        searchPhoneNumber: this.searchPhoneNumber
      }).then(response => {
        this.pageList = []
        this.pageList = response.data.data.list
        this.pagination.total = response.data.data.total
        for (let i = 0; i < response.data.data.list.length; i++) {
          this.pageList[i].apiTradeNotification = this.pageList[i].apiTradeNotification.toString()
          this.pageList[i].banned = this.pageList[i].banned.toString()
          this.pageList[i].secondHandNotification = this.pageList[i].secondHandNotification.toString()
          this.pageList[i].agentServiceNotification = this.pageList[i].agentServiceNotification.toString()
        }
      })
        .catch(err => {
          console.log(err)
        })
    },

    async handleUpdate(row) {
      this.resetForm()
      this.dialog = {
        title: '修改用户',
        visible: true
      }
      // await this.loadRoleOptions()
      // await this.loadDeptOptions()
      const id = row.id || this.ids
      ban(id).then(response => {
        this.form = response.data
      })
    },
    handleSubmit: function() {
      const username = this.form.username
      if (this.checkName(username)) {
        add(this.form).then(() => {
          this.$message.success('add successfully')
          this.dialog.visible = false
          this.$router.go(0)
        })
      }
    },
    checkName(username) {
      for (var i in this.pageList) {
        if (this.pageList[i].username === username) {
          this.$message.error('the username has been used')
          return false
        }
      }
      return true
    },
    banUser(row) {
      const ids = row.id
      this.$confirm('Do you want to ban this user?', 'warning', {
        confirmButtonText: 'confirm',
        cancelButtonText: 'cancel',
        type: 'Warning'
      }).then(function() {
        return ban(ids)
      }).then(() => {
        this.$message.success('ban successfully')
        this.$router.go(0)
        // this.handleQuery()
      }).catch(() =>
        this.$message.info('cancelled')
      )
    },
    unbanUser(row) {
      const ids = row.id
      this.$confirm('Do you want to unban this user?', 'warning', {
        confirmButtonText: 'confirm',
        cancelButtonText: 'cancel',
        type: 'Warning'
      }).then(function() {
        return unban(ids)
      }).then(() => {
        this.$message.success('unban successfully')
        this.$router.go(0)
        // this.handleQuery()
      }).catch(() =>
        this.$message.info('cancelled')
      )
    },
    async loadData() {
      list({ pageNo: this.pagination.page, pageSize: this.pagination.limit }).then(response => {
        if (response.data.errCode === 401) {
          removeToken()
          this.$router.go(0)
          console.log('token has expired')
        }
        this.pageList = []
        this.pageList = response.data.data.list
        this.pagination.total = response.data.data.total
        for (let i = 0; i < response.data.data.list.length; i++) {
          this.pageList[i].apiTradeNotification = this.pageList[i].apiTradeNotification.toString()
          this.pageList[i].banned = this.pageList[i].banned.toString()
          this.pageList[i].secondHandNotification = this.pageList[i].secondHandNotification.toString()
          this.pageList[i].agentServiceNotification = this.pageList[i].agentServiceNotification.toString()
        }
      })
        .catch(err => {
          /*        removeToken()
                      this.$router.go(0)*/
          console.log(err)
        })
    },

    resetForm() {
      this.form = {
        id: undefined,
        username: undefined,
        /*        mobile: undefined,
                email: undefined,
                gender: undefined,
                remark: undefined,*/
        roleIds: undefined
      }
      if (this.$refs['form']) {
        this.$refs['form'].resetFields()
      }
    }
  }
}
</script>
